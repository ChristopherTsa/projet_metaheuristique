...
@njit
def insertion_heuristic(N, M, profits, resource_consumption, resource_availabilities):
    """
    Implements the insertion heuristic for the multidimensional knapsack problem.
    ...
    # Start with an empty solution and iteratively add projects
    for j in sorted_indices:
        # Check if adding project j would make the solution infeasible
        fits = True
        for i in range(M):
            if used_resources[i] + resource_consumption[i, j] > resource_availabilities[i]:
                fits = False
                break
        # Add project j if it fits
        if fits:
            solution[j] = 1
            for i in range(M):
